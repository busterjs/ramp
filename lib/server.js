...
module.exports = {
    ...
    create: function () {
        ...
        server._resourceMiddleware = rampResources.resourceMiddleware.create();
        server._resourceCache = rampResources.resourceSetCache.create();
    ...
    _setHeader: function (data) {
        rampResources.resourceSet.deserialize(data.resourceSet).then(function (rs) {
            this.hasHeader = true;
            this.headerHeight = data.height;
            this.headerResourceSet = rs;
        }.bind(this));
    },
    ...
    _respond: function (req, res) {
        ...
        if (req.method == "POST" && url.path == "/header") {
            concatReqBody(req).then(function (body) {
                this._setHeader(JSON.parse(body));
            }.bind(this));
            return true;
        }
