...
module.exports = {
    create: function () {
        ...
        instance._pubsubClient = bCapServPubsubClient.create({
            fayeClient: instance._pubsubServer.getClient()
        });

        instance._sessionQueue.on("slave:captured", function (serializedSlave) {
            instance._onSlaveCaptured(serializedSlave);
        });

        instance._sessionQueue.on("slave:freed", function (serializedSlave) {
    ...
    _attachSlave: function (slave) {
        slave.attach(this._httpServer, this._pubsubServer);
    },
