...
module.exports = {
    ...
    create: function () {
        ...
        server._pubsubClient = server._pubsubServer.createClient({escapeEventNames: true});
    ...
    _onSlaveCaptured: function (slave) {
        this._pubsubClient.publish("slave:captured", {
    ...
    _onSlaveFreed: function (slave) {
        this._pubsubClient.publish("slave:freed", {
            slave: slave.serialize(),
            slaves: this._slaves().map(function (s) { return s.serialize() })
        });
        slave.teardown();
    ...
    _onPrepareSession: function (session) {
        ...
        this._resourceCache.inflate(session.resourceSet).then(function (rs) {
            ...
            process.nextTick(function () {
                self._pubsubClient.publish("session:started", session.serialize());
            });
        }.bind(this));
    ...
    _onTeardownSession: function (session) {
        ...
        session.teardown();
        process.nextTick(function () {
            self._pubsubClient.publish("session:ended", session.serialize());
        });
    },
