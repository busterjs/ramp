...
var baseSessionRS = rampResources.resourceSet.create();
...
module.exports = buster.extend(buster.eventEmitter.create(), {
    create: function (sessionData, pubsubServer) {
        ...
        instance.clientPubsubConnectionId = sessionData.pubsubConnectionId;
    ...
    _pubsubServerAttach: function (pubsubServer) {
        ...
        this._privatePubsubClient.on("initialize", function (e) {
            ...
            if (!("_pubsubServer" in this)) return;

            this._emitState();
        }.bind(this));

        this.pubsubServerDisconnectHandler = function (clientId) {
            if (clientId === this.clientPubsubConnectionId) {
                this._end();
            }
        }.bind(this);

        pubsubServer.on("client:disconnect",
                        this.pubsubServerDisconnectHandler);
    },

    _pubsubServerDetach: function () {
        if (!this._pubsubServer) { return; }

        this._pubsubServer.removeListener("client:disconnect",
                                          this.pubsubServerDisconnectHandler);
