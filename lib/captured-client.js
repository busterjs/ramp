...
module.exports = buster.extend(buster.eventEmitter.create(), {
    create: function (server, busterResources, headerResourceSet, headerHeight, currentSession) {
        ...
        client._bayeuxDisconnectListener = function (clientId) {
            if (client.bayeuxClientId == clientId) client.end();
        };
        server.bayeuxServer.bind("disconnect", client._bayeuxDisconnectListener);
        ...
        if (currentSession && !(currentSession.joinable == false)) {
            client.startSession(currentSession);
        }
