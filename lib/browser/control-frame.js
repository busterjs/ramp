...
(function () {
    ...
    buster.server.controlFrame = {
        listen: function () {
            var self = this;
            this.sessionFrame = document.getElementById("session_frame");
        ...
        sessionStart: function (session) {
            ...
            this.sessionFrame.src = session.resourceContextPath + "/";
            buster._captureCrossBrowserUtil.addEventListener(this.sessionFrame, "load", function () {
                setTimeout(function () {
                    self.sessionFrame.contentWindow.focus();
                    readyHandler();
                }, 1);
            });
        },
        ...
        sessionEnd: function (event) {
            this.sessionFrame.src = "";
        },

        exposeBusterObject: function (session, cb) {
            var self = this;
            var busterObj = this.sessionFrame.contentWindow.buster;
