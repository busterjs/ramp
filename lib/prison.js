(function (B) {
    ...
    B.captureServer.prison = {
        create: function () {
            var server = B.create(this);
            ...
            server.slaveId = /^[a-z]+:\/\/[^\/]+\/slaves\/([^\/]+)/.exec(l)[1];
            server.capturePath = "http://" + host + ":" + port + "/capture";

            server.serverClient = B.captureServer.pubsubClient.create({
                host: host,
                port: port
            });
