...
var DEFAULT_SLAVE_IDLE_RESOURCE_SET = rampResources.createResourceSet();
...
function createSession(rampServer, logger, req, res) {
    ...
    var reqBody;
    bufferUtils.concatHttpBody(req)
        .then(function (bodyText) {
            return JSON.parse(bodyText.toString("utf8"));
        })
        .then(function (parsedBody) {
            reqBody = parsedBody;
            logger.info("Creating new session: Deserializing resource set");
            return rampResources.deserialize(reqBody.resourceSetValue);
        })
        .then(function (resourceSet) {
            logger.info("Creating new session: Inflating resource set from cache");
            return mori.get(rampServer, "resourceCache").inflate(resourceSet);
        })
        .then(function (resourceSet) {
            return rampSession.createSession(resourceSet, reqBody.rampClientId, reqBody.options);
        })
        .then(function (session) {
            var sessionPublicValue = rampSession.toPublicValue(session);
            atoms.rampClientHeartbeats = mori.assoc(atoms.rampClientHeartbeats, reqBody.rampClientId, new Date().getTime());
...
function initializeSession(rampServer, logger, req, res) {
    ...
    writeJson(res, {
        ...
        initialSlaves: mori.intoArray(mori.map(rampSlave.toPublicValue, activeSlaves))
    });
}
...
function respond(rampServer, logger, req, res) {
    ...
    if (req.method === "GET" && url.path.indexOf("/capture") === 0) {
    ...
    if (atoms.currentSession) {
        match = rampSession.matchPathForSession(atoms.currentSession, url.path);
        if (match) {
            var segment = match[1];
    ...
    match = url.path.match(/^\/slaves\/([^\/]+)\/chains$/);
    if (match) {
        var slaveId = match[1];
    ...
    if (mori.get(rampServer, "resourceMiddleware").respond(req, res)) {
        return true;
    }
}
...
function removeSlave(rampServer, slave) {
    ...
    if (atoms.currentSession) {
        ...
        if (mori.hasKey(sessionSlaves, slave)) {
...
function checkSlaveTimeouts(rampServer, logger) {
    ...
    setTimeout(function () {
        ...
        mori.each(deadSlaves, function (heartBeat) {
            ...
            mori.get(rampServer, "fayeClient").publish("/slave_death", { slaveId: slaveId });
...
function onSlaveHeartbeat(rampServer, logger, e) {
    ...
    if (mori.hasKey(atoms.allSlaves, slaveId)) {
...
function getActiveSlaves(rampServer, logger) {
    var atoms = mori.get(rampServer, "atoms");
    return mori.intoArray(mori.map(rampSlave.toPublicValue, atoms.activeSlaves));
}
...
module.exports.createServer = function (opts) {
    ...
    if (opts.veryVeryVerbose) {
        ...
        function logFayeMessage(prefix, msg) {
            if (msg.subscription === "/ramp_client_heartbeat") {
                return;
            }
            if (msg.subscription === "/slave_heartbeat") {
                return;
            }
            logger.debug(prefix, msg);
        }
    ...
    var rampServer = mori.hashMap(
        "atoms", {
            allSlaves: mori.hashMap(),
            slaveHeartbeats: mori.hashMap(),
            activeSlaves:  mori.set(),
            loadingSlaves: mori.set(),
            rampClientHeartbeats: mori.hashMap()
        },
    ...
    if ("header" in opts) {
        ...
        if (!header.resourceSet) {
            throw new Error("Expected header option to specify 'resourceSet'.");
        }
        if (!header.height) {
            throw new Error("Expected header option to specify 'height'.");
        }

        var headerPath = "/slave_header/";

        rampServer = mori.assoc(
            rampServer,
            "header", mori.hashMap(
        ...
        rampSlave.processResourceSet(header.resourceSet).then(function (processedResourceSet) {
            mori.get(rampServer, "resourceMiddleware").mount(headerPath, processedResourceSet);
        });
    }
