...
function concatBody(req, res) {
    ...
    req.on("end", function () {
        try {
            deferred.resolve(JSON.parse(concatBuffers(buffers).toString("utf8")));
        } catch (e) { deferred.reject("JSON parse error: " + e.message); }
    })
...
module.exports.createServer = function (opts) {
    ...
    fayeAdapter.addExtension({
        incoming: function (msg, callback) {
            logger.debug("[BAYEUX IN ] ", msg);
            callback(msg);
        },
        outgoing: function (msg, callback) {
            logger.debug("[BAYEUX OUT] ", msg);
            callback(msg);
        }
    })
