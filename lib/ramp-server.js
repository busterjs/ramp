...
function performCapture(rampServer, req, res) {
    ...
    mori.get(rampServer, "atoms").slaveHeartbeats = mori.assoc(mori.get(rampServer, "atoms").slaveHeartbeats, mori.get(slave, "id"), new Date().getTime() + mori.get(rampServer, "slaveLoadTimeHint"));
...
function removeSlave(rampServer, slave) {
    mori.get(rampServer, "atoms").slaveHeartbeats = mori.dissoc(mori.get(rampServer, "atoms").slaveHeartbeats, mori.get(slave, "id"));
    mori.get(rampServer, "atoms").allSlaves = mori.dissoc(mori.get(rampServer, "atoms").allSlaves, mori.get(slave, "id"));
    mori.get(rampServer, "atoms").activeSlaves = mori.disj(mori.get(rampServer, "atoms").activeSlaves, slave);
    mori.get(rampServer, "atoms").loadingSlaves = mori.disj(mori.get(rampServer, "atoms").loadingSlaves, slave);

    mori.get(rampServer, "resourceMiddleware").unmount(mori.get(slave, "chainsPath"));

    var fayeAdapter = mori.get(rampServer, "fayeAdapter");
    var fayeClientId = mori.get(slave, "fayeClientId")
    if (fayeClientId) {
        fayeAdapter._server._engine.destroyClient(fayeClientId);
    }
};
...
function checkSlaveTimeouts(rampServer) {
    ...
    setTimeout(function () {
        ...
        }, mori.get(rampServer, "atoms").slaveHeartbeats);

        mori.each(deadSlaves, function (heartBeat) {
            var slaveId = mori.get(heartBeat, 0);
            var lastSeen = mori.get(heartBeat, 1);

            var slave = mori.get(mori.get(rampServer, "atoms").allSlaves, slaveId);
...
module.exports.createRampServer = function (opts) {
    ...
    mori.get(rampServer, "atoms").slaveHeartbeats = mori.hash_map();
    ...
    fayeAdapter.getClient().subscribe("/slave_heartbeat", function (e) {
        // We ignore heartbeats from dead slaves
        if (!mori.has_key(mori.get(rampServer, "atoms").allSlaves, e.slaveId)) return;

        mori.get(rampServer, "atoms").slaveHeartbeats = mori.assoc(mori.get(rampServer, "atoms").slaveHeartbeats, e.slaveId, new Date().getTime());
    });
