...
module.exports = {
    ...
    create: function (data) {
        ...
        for (var key in data.resources) {
            session.resources[key] = resource.create(key, data.resources[key]);
        }
        ...
        if (!("/" in session.resources)) {
            session.resources["/"] = resource.create("/", {
        ...
        p.scripts = session.rootResourceScripts();
        session.resources["/"].addProcessor(p);
