...
module.exports = {
    ...
    respond: function (req, res) {
        if (req.method == "GET" && req.url == "/" && (/application\/json/).test(req.headers.accept)) {
            respondWithMetadata.call(this, res);
            return true;
        }
...
function setupBayeux() {
    ...
    this.bayeuxServer.bind("disconnect", function (clientId) {
        if (clientId == self.sessionOwnerFayeClientId) {
            self.session.destroyCurrentSession();
            return;
        }
...
function respondWithMetadata(res) {
    res.writeHead(200);
    res.write(JSON.stringify({
        messagingPath: this.messagingContextPath
    }));
    res.end();
}