...
module.exports = {
    captureSlave: function (port, ua) {
        ...
        var rCapServ = require("./ramp-capture-server");
        var rCapServPubsubClient = require("./pubsub-client");

        ua = ua || "Whatever";
        var deferred = when.defer();
        var self = this;

        var mockBrowserPubsub = rCapServPubsubClient.create({
            host: "localhost",
            port: port
        });

        var c = rCapServ.createServerClient(port);
