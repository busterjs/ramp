...
module.exports = {
    ...
    createClient: function (res) {
        ...
        client.messagingUrl = client.multicastMiddlewareClient.url;
    ...
    startSession: function (session) {
        for (var i = 0, ii = this.clients.length; i < ii; i++) {
            this.clients[i].multicastMiddlewareClient.emit([
    ...
    endSession: function () {
        for (var i = 0, ii = this.clients.length; i < ii; i++) {
            this.clients[i].multicastMiddlewareClient.emit([{topic: "session:end"}]);
        }
    }
};