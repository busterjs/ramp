...
module.exports = {
    ...
    bindToSessionMiddleware: function (sessionMiddleware) {
        ...
        sessionMiddleware.on("session:end", function (session) {
            self.endSession();
        });
    },
    ...
    startSession: function (session) {
        for (var i = 0, ii = this.clients.length; i < ii; i++) {
            this.clients[i].messagingMiddlewareClient.emit([
    ...
    endSession: function () {
        for (var i = 0, ii = this.clients.length; i < ii; i++) {
            this.clients[i].messagingMiddlewareClient.emit([{topic: "session:end"}]);
        }
    }
};