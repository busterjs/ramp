...
module.exports = {
    create: function (server, resourceMiddleware, headerResourceSet, headerHeight) {
        ...
        createResourceSet.call(client, resourceMiddleware);
...
function createResourceSet(resourceMiddleware) {
    var self =  this;
    ...
    resourceSet.addResource("/env.js", {
        content: function (promise, req) {
            promise.resolve("var buster = buster || {}; buster.env = " + getEnv.call(self, req) + ";");
        }
    });
...
function getEnv(req) {
    ...
    var env = {
        clientId: this.id,
        bayeuxUrl: "http://" + host + this.server.messagingContextPath,
        captureUrl: "http://" + host + this.server.capturePath
    };
