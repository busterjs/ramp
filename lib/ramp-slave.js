...
var BASE_HEADER_SCRIPTS = [
    require.resolve("./ramp-slave-chains-header-frame-initializer.js")
];
var BASE_HEADER_SCRIPT_NAME = "/____ramp_header_internals-" + uuid() + ".js";
var BASE_HEADER_RESOURCE_SET = rampResources.createResourceSet();
BASE_HEADER_RESOURCE_SET.addResources(BASE_HEADER_SCRIPTS.map(function (lib) {
    return {path: lib, content: fs.readFileSync(lib)}
}))
BASE_HEADER_RESOURCE_SET.addResource({path: BASE_HEADER_SCRIPT_NAME, combine: BASE_HEADER_SCRIPTS});
BASE_HEADER_RESOURCE_SET.loadPath.append(BASE_HEADER_SCRIPT_NAME);
