...
var templates = {
    ...
    sessionReset: fs.readFileSync(__dirname + "/templates/session_reset.html", "utf8")
};
...
exports.create = function (server, bayeuxServer, resourceSet, headerResourceSet, headerHeight, currentSession) {
    ...
    var slave = buster.extend(buster.eventEmitter.create(), {
        respond: function (req, res, pathname) {
            if (req.method == "GET" && pathname == this.url) {
                ...
                locals.scripts = this.resourceSet.load;
            ...
            if (req.method == "GET" && pathname == this.sessionResetPath) {
                res.writeHead(200, {"Content-Type": "text/html"});
                res.write(templates.sessionReset);
                res.end();
                return true;
            }
        },
        ...
        getEnv: function() {
            var env = {
                ...
                sessionResetPath: this.sessionResetPath
            };
    ...
    slave.sessionResetPath = slave.url + "/session-reset";
    slave.env = {};
