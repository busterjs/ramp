...
exports.create = function (server, bayeuxServer, resourcesMiddleware, sessionQueue, userAgent, logger) {
    ...
    function broadcastSession() {
        var session = sessionQueue.currentSession();
        if (!session) return;

        bayeuxClient.publish("/" + id + "/session/start", session.serialize());

        logger.info("Session " + session.id + " about to start in " + slave.userAgent);
        var readyEvent = "/" + id + "/session/" + session.id + "/ready";
    ...
    function unloadSession(session) {
